...
class WatchConnectivity {
  ...
  /// Initialize the platform SDK. If the selected [WatchType] requires this,
  /// it must be called before any other operations.
  /// 
  /// Required for watch types:
  /// - [WatchType.garmin]
  Future<void> initialize(WatchInitializationOptions options) async {
    await _channel.invokeMethod('initialize', options.toJson());
  }
...
enum WatchType {
  ...
  /// Garmin watch
  ///
  /// Requires the `watch_connectivity_garmin` plugin and a call to
